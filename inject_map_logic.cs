...
";
        ...
        int insertIndex = js.IndexOf("map.on('click', (e) => {");
        if (insertIndex > -1) {
            string clickLogic = @"
        if (activeTacticalMode) {
            addTacticalPoint(activeTacticalMode, e.latlng);
            return;
        }
";
            int bodyStart = js.IndexOf("{", insertIndex) + 1;
            js = js.Insert(bodyStart, clickLogic);

            js = js.Insert(insertIndex, newLogic + "\n");
            File.WriteAllText(path, js, new UTF8Encoding(false));
            Console.WriteLine("Injected UI map handlers.");
        } else {
            Console.WriteLine("Could not find map click event.");
        }
    }
}
